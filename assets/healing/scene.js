// scene.js — capa Scene (nieve + música)
export function startSnow(canvas){
  if(!canvas) return;

  const ctx = canvas.getContext("2d", { alpha:true });

  // Cartoon 2D + performance (sprites pre-render + caps)
  const DPR_CAP = 1.5;
  ...
  // Sprites "cartoon": sólidos + contorno (menos blur)
  const SPR = 96;
  ...
  // +30% velocidad (vs tu versión anterior)
  ...
  function buildSprites(){
    ...
    // SNOW: copito "asterisco" cartoon (blanco con contorno gris)
    sctx.clearRect(0,0,SPR,SPR);
    sctx.save();
    sctx.translate(cx, cy);

    // halo MUY leve para presencia (sin bokeh)
    ...
    // PETAL: gotita/hojita rosa cartoon con contorno
    pctx.clearRect(0,0,SPR,SPR);
    pctx.save();
    pctx.translate(cx, cy);
    pctx.rotate(-0.35);

    // sombra suave mínima (no blur)
  ...
  function makePart(spawnAnywhere=true){
    ...
    if(!isPetal){
      // nieve: un poquito más “copito” (no blobs)
      w = h = 5.0 + Math.random()*7.5;          // 5..12.5
      a = 0.55 + Math.random()*0.35;            // 0.55..0.90 (presencia)
      vy = (14.5 + Math.random()*20.0) * SPEED_MUL; // base +30%
      vx = (-10 + Math.random()*20);
      rotV = (-0.8 + Math.random()*1.6);
      wob = 1.8 + Math.random()*2.8;
      twk = 0.7 + Math.random()*1.0;
    }else{
      // pétalo: más definido y visible
      w = 9.0 + Math.random()*13.0;             // 9..22
      h = w * (0.46 + Math.random()*0.20);      // 0.46..0.66 (pétalo)
      a = 0.70 + Math.random()*0.28;            // 0.70..0.98
      vy = (12.0 + Math.random()*17.0) * SPEED_MUL; // base +30%
  ...
  function step(now){
    ...
    for(let i=0;i<parts.length;i++){
      ...
      // twinkle leve (cartoon: casi constante)
...
// Música (3 tracks, play/pause, shift+click next, click derecho prev)
export function createMusicController(buttonEl, tracks){
  let musicIndex = Math.max(0, Math.min(tracks.length-1, parseInt(localStorage.getItem("nivalis_music_index")||"0",10) || 0));
  let musicOn = false;
  ...
  function loadTrack(idx){
    ...
    localStorage.setItem("nivalis_music_index", String(musicIndex));
